...
def main():
    ...
    if args.config == None:
        raise OSError("path to config fle must be required.")
    ...
    for idx, img_name in enumerate(sorted(os.listdir(args.img_dir))):
        ...
        save_path = os.path.join(args.output_dir, 'result_{}.png'.format(img_name.replace('.png', '')))
